...
# Полиморфизм методов
class A:
    # атрибут (свойства) определенное без конструктора
    attr_1 = 100 
    def m1(self, arg):
...
# Полиморфизм с "магическими" методами (методами перезагрузки операторов)
...
# инкапсулация - скрытие атрибутов и/или методов
# чтобы они не были доступны из экземпляров класса

# не строгая инкапсулация
...
class Neuron:
    ...
    def __call__(self, data):
        return self.weihts[0] * data[0] + self.weihts[1] * data[1]
